...
class APIFeatures {
  ...
  if (queryObj.roomType === "Room" || queryObj.roomType === "Any Type") {
    // Don't filter by room type
  } else if (queryObj.roomType === "Entire Home") {
    filterQuery.roomType = "entire place";
  } else {
    filterQuery.roomType = queryObj.roomType.toLowerCase();
  }
}
  ...
  const normalizedAmenities = amenitiesArray.map((amenity) => {
    if (amenity.toLowerCase() === "wifi") return "Wifi";
    if (amenity.toLowerCase() === "ac") return "Ac";
    if (amenity.toLowerCase() === "kitchen") return "Kitchen";
    if (amenity.toLowerCase() === "tv") return "Tv";
    if (amenity.toLowerCase() === "pool") return "Pool";
    if (amenity.toLowerCase() === "washing machine") return "Washing Machine";
    if (amenity.toLowerCase() === "free parking") return "Free Parking";
    if (amenity.toLowerCase() === "hot tub") return "Hot Tub";
    if (amenity.toLowerCase() === "gym") return "Gym";

    return amenity;
  });

  filterQuery["amenities.name"] = { $all: normalizedAmenities };
}
